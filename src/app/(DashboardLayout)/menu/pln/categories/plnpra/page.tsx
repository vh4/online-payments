...
import InvoicePLNPra from "@/app/(DashboardLayout)/components/invoice/plnpraInvoice";
...
export default function PaymentPage() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const result = await payPlnPra(
        inquiryData.idpel1,
        parseInt(inquiryData.nominal)
      );
      if (result.status !== "00") {
        toaster.push(
          <Notification
    ...
    <div className="flex justify-center">
      <div className="w-full">
        {success ? (
          ...
          <>
            <PageContainer title="PLN categories Checkout" description="PLN Prepaid Checkout">
              <Grid container spacing={3}>
                <Grid item sm={12}>
                  <DashboardCard title="PLN Prepaid checkout">
                    <div>
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td className="p-2 text-gray-800">
                                  {dataLoaded ? (
                                    `${
                                      inquiryData.subscribersegmentation
                                    } / ${parseInt(
                                      inquiryData.powerconsumingcategory
                                    )}`
                                  ) : (
